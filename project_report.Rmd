...
#### First, we looked into which variables were the most complete. The following table shows the % completeness for all variables. Across the `r nrow(met_10)` observations, `object_name`, `accession_year`, and `department` were most complete. This informed which analytic questions we could answer.
...
```{r }
data.frame(sapply(met, function(x) round(sum(!is.na(x))/nrow(met_10)*100,2))) %>%
